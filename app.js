...
function switch_panel(panel) {
  ...
  if (panel === "editor") {
    ...
    admin_users_page=1;load_admin_users();load_whitelist();load_scripts();
  }
}
...
async function load_scripts() {
  try {
    const data = await api_get("/scripts");
    if (!data.success) return;
    
    render_scripts(data.scripts);
  } catch (err) {
...
function render_scripts(scripts) {
  ...
  scripts.forEach(script => {
    const item = document.createElement("div");
    item.style.cssText = "display:flex;align-items:center;justify-content:space-between;padding:10px 12px;background:var(--bg3);border:1px solid var(--border);border-radius:8px;cursor:pointer;transition:background 0.2s";
    item.onclick = () => edit_script(script.name);
    
    item.innerHTML = `
      <div>
        <span style="font-size:12px;color:var(--text);font-weight:500">${esc(script.name)}</span>
        ${script.description ? `<span style="font-size:11px;color:var(--muted);margin-left:8px">${esc(script.description)}</span>` : ''}
      </div>
      <span style="font-size:10px;color:var(--muted2)">${new Date(script.updatedAt).toLocaleDateString()}</span>
    `;
    list.appendChild(item);
  });
}

async function create_script() {
  const input = document.getElementById("script_name_input");
  const name = input.value.trim();
  
  if (!name) {
    show_toast("Please enter a script name", "error");
    return;
  }
  
  try {
    const data = await api_post("/scripts", { name, code: "// Your code here", description: "" });
    if (data.success) {
      show_toast("Script created", "success");
      input.value = "";
      load_scripts();
      edit_script(name);
    } else {
      show_toast(data.error || "Failed to create script", "error");
    }
  } catch (err) {
    console.error("Failed to create script:", err);
    show_toast("Failed to create script", "error");
  }
}

async function edit_script(name) {
  try {
    const data = await api_get(`/scripts/${encodeURIComponent(name)}`);
    if (!data.success) {
      show_toast("Failed to load script", "error");
      return;
    }
    
    currentEditingScript = name;
    document.getElementById("editing_script_name").textContent = name;
    document.getElementById("script_code").value = data.script.code;
    document.getElementById("script_description").value = data.script.description || "";
    document.getElementById("script_output").style.display = "none";
    document.getElementById("script_editor").style.display = "block";
  } catch (err) {
...
async function save_script() {
  ...
  const code = document.getElementById("script_code").value;
  const description = document.getElementById("script_description").value;
  
  if (!code) {
    show_toast("Code cannot be empty", "error");
    return;
  }
  
  try {
    const data = await api_post(`/scripts/${encodeURIComponent(currentEditingScript)}`, { code, description }, "PUT");
    if (data.success) {
      show_toast("Script saved", "success");
      load_scripts();
    } else {
      show_toast(data.error || "Failed to save script", "error");
    }
  } catch (err) {
    console.error("Failed to save script:", err);
    show_toast("Failed to save script", "error");
  }
}
...
async function execute_script() {
  ...
  const outputDiv = document.getElementById("script_output");
  outputDiv.style.display = "block";
  outputDiv.textContent = "Executing...";
  
  try {
    const data = await api_post(`/scripts/${encodeURIComponent(currentEditingScript)}/execute`, {});
    if (data.success) {
      outputDiv.textContent = `Output: ${data.output}`;
      show_toast("Script executed", "success");
    } else {
      outputDiv.textContent = `Error: ${data.error}`;
      show_toast("Script execution failed", "error");
    }
  } catch (err) {
    console.error("Failed to execute script:", err);
    outputDiv.textContent = `Error: ${err.message}`;
    show_toast("Failed to execute script", "error");
  }
}

async function delete_script() {
  if (!currentEditingScript) return;
  
  if (!confirm(`Are you sure you want to delete "${currentEditingScript}"?`)) return;
  
  try {
    const data = await api_post(`/scripts/${encodeURIComponent(currentEditingScript)}`, {}, "DELETE");
    if (data.success) {
      show_toast("Script deleted", "success");
      close_script_editor();
      load_scripts();
    } else {
      show_toast(data.error || "Failed to delete script", "error");
    }
  } catch (err) {
    console.error("Failed to delete script:", err);
    show_toast("Failed to delete script", "error");
  }
}
